...
public class Editor{
	...
	public GridPane getLayout(){
		...
		canvasPane.addEventFilter(javafx.scene.input.ScrollEvent.SCROLL, e -> {
			if (this.selectedImage.getImage() != null){
				e.consume();
			}
		});
		canvasPane.addEventFilter(javafx.scene.input.MouseEvent.MOUSE_DRAGGED, e -> {
			if (this.selectedImage.getImage() != null){
				e.consume();
			}
		});
		...
		unselect.setOnAction(e -> this.selectedImage.setImage(null));
		Button saveLocal = new Button("Save locally");
		saveLocal.setOnAction(e -> {
			this.saveFile(FXActivity.getInstance().getExternalFilesDir(null).getAbsolutePath());
			Alert alert = new Alert(Alert.AlertType.INFORMATION);
			alert.setHeaderText("File saved");
			alert.setContentText("File saved successfully");
			alert.showAndWait();
		});
		Button loadLocal = new Button("Load locally");
		loadLocal.setOnAction(e -> {
			try {
				for (File f : FXActivity.getInstance().getExternalFilesDir(null).listFiles()){
					File target = new File(saveDirectory, f.getName());
			...
			Alert alert = new Alert(Alert.AlertType.INFORMATION);
			alert.setHeaderText("Files loaded");
			alert.setContentText("Files loaded successfully");
			alert.showAndWait();
		});
		tools.getItems().addAll(newB, save, open, exit, run, delete, unselect, saveLocal, loadLocal);
	...
	private void updatePropsLayout(){
		...
		if (this.clickSelected == null){
			...
			apply.setOnAction(e -> {
				if (name.getText().equals(this.saveFileName)){
	...
	private void saveFile(String targetDir){
		lastFile = this.saveFileName;
		File file = new File(targetDir, lastFile);
		try {
			if (!file.exists()){
				file.createNewFile();
			}
